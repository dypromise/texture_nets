...
function M.RandomCrop(size, padding)
  ...
  return function(input)
    local size_w = math.min(input:size(3), size)
    local size_h = math.min(input:size(2), size)

    if padding > 0 then
      local temp = input.new(3, input:size(2) + 2 * padding, input:size(3) + 2 * padding)
      temp:zero():narrow(2, padding + 1, input:size(2)):narrow(3, padding + 1, input:size(3)):copy(input)
      input = temp
    end
...
function M.Lighting(alphastd, eigval, eigvec)
  return function(input)
    ...
    local rgb = eigvec:clone():cmul(alpha:view(1, 3):expand(3, 3)):cmul(eigval:view(1, 3):expand(3, 3)):sum(2):squeeze()
