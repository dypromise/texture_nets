...
assert(
  params.mode == "style",
  "Only stylization is implemented in master branch.You can find texture" .. "generation in texture_nets_v1 branch."
)
...
if params.pyramid_loss > 1 then
  ...
  for i = 2, params.pyramid_loss do
    ...
    s = s * 2
    local local_texture_image =
      image.scale(texture_image, texture_image:size(3) / s, texture_image:size(2) / s, "bicubic"):float()
    criterion:add(
      nn.PreprocessCriterion(
        nn.ArtisticCriterion(params, cnn:clone(), local_texture_image:clone()),
        nn.SpatialAveragePooling(s, s, s, s)
      ),
      w
    )
  end
